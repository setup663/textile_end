        ...
        try:
            ...
            with self.db_manager as db:
                ...
                for material, data in total_fabric_required.items():
                    print("total_fabric_required.items()", total_fabric_required.items())
                    ...
                    for material, shortage in self.shortage_data.items():
                        ...
                        for row in rows:
                            ...
                            oc_res = db.execute_query(
                                "SELECT pm.id, oc.id as oc_id FROM order_composition oc JOIN product_materials pm ON oc.id = pm.order_composition_id JOIN material m ON pm.supply_composition_id = m.id WHERE oc.order_id = %s AND m.name = %s LIMIT 1",
            ...
            with self.db_manager as db:
                for material_name, shortage in self.shortage_data.items():
                    ...
                    sc_id = db.execute_insert(
                        "INSERT INTO supply_composition (supply_id, material_id, quantity, length, width, cost, unit_quantity, status, location_id, remainder) VALUES (%s, %s, %s, NULL, NULL, NULL, NULL, %s, NULL, %s)",
