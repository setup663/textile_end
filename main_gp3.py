...
class Main(QtWidgets.QWidget, Ui_Form):
    def __init__(self, parent=None):
        ...
        self.hardware_shortage = {}  # для фурнитуры (через material_type.name = 'Фурнитура')
    ...
    def show_order_info(self, order):
        try:
            ...
            with self.db_manager as db:
                ...
                # Загрузка данных по изделиям заказа (подсчёт общего количества изделий и площади)
                order_query = """
                ...
                # Не вызываем check_and_prompt_supply_request здесь – дополнительные окна появятся только при нажатии на pushButton_calculate_rascr.
        except Exception as e:
    ...
    def calculate_cutting(self):
        ...
        появляются только в этом методе.
        """
        ...
        try:
            ...
            with self.db_manager as db:
                ...
                for material, items in items_by_material.items():
                    ...
                    while any(item['quantity'] > 0 for item in items_copy):
                        ...
                        if placements:
                            ...
                            # Исправляем уменьшение количества: распределяем вычитание по всем записям с одинаковым именем
                            for u in used:
                                remaining = u['count']
                ...
                logging.debug(f"Available fabrics: {available_dict}")
